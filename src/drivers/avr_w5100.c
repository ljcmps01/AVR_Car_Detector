...
//   W51_write(sockaddr + W5100_CR_OFFSET, W5100_SKT_CR_OPEN); // open the
//   socket
...
//   if (sock > W5100_NUM_SOCKETS)
//     return;                        // if illegal socket number, ignore
//     request
//   sockaddr = W5100_SKT_BASE(sock); // calc base addr for this socket
...
//     else
//       CloseSocket(sock); // not in listen mode, close and show an error
//       occurred
//   }
//   return retval;
// }
...
//                             // Make sure the TX Free Size Register is
//                             available
//   txsize = W51_read(
//       sockaddr +
//       W5100_TX_FSR_OFFSET); // make sure the TX free-size reg is available
//   txsize =
//       (((txsize & 0x00FF) << 8) + W51_read(sockaddr + W5100_TX_FSR_OFFSET +
//       1));
...
//         W5100_TXBUFADDR +
//         (offaddr &
//          W5100_TX_BUF_MASK); // calc W5100 physical buffer addr for this
//          socket

//     W51_write(realaddr, *buf); // send a byte of application data to TX
//     buffer offaddr++;                 // next TX buffer addr buf++; // next
//     input buffer addr
//   }

//   W51_write(sockaddr + W5100_TX_WR_OFFSET,
//             (offaddr & 0xFF00) >> 8); // send MSB of new write-pointer addr
//   W51_write(sockaddr + W5100_TX_WR_OFFSET + 1, (offaddr & 0x00FF)); // send
//   LSB
...
//   sockaddr = W5100_SKT_BASE(sock); // calc base addr for this socket
//   ptr =
//       W51_read(sockaddr + W5100_RX_RD_OFFSET); // get the RX read pointer
//       (MSB)
//   offaddr =
...
//  *  allows the library code to invoke a target-specific function for
//  disabling
//  *  the W5100 chip.
//  */
...
// /*
//  *  my_reset      callback function; force a hardware reset of the W5100
//  device
//  */
// void my_reset(void) {
...
//   my_callbacks._deselect = &my_deselect; // callback for deselecting the
//   W5100 my_callbacks._reset = &my_reset; // callback for hardware-reset of
//   the W5100
